;(function(factory) {
	...
	_v[ns] = function($el, options) {
		...
		// 	3. Default plugin options defined in $.fn.pluginName.options object.
		this.options = $.extend(true, {}, $.fn[ns].options, options, $el.data(ns));
	 ...
	 */
	function get($el, opt) {
		var o = $.data($el[0], ns + '_plugin');
		if (!o) {
			o = new _v[ns]($el, opt);
			$.data($el[0], ns + '_plugin', o);
		}
		return o;
	}
	...
	$.fn[ns] = function(options) {
		...
		if (typeof(options) == 'string' && options.charAt(0) != '_') {
			...
			if (options == 'destroy') {
				return this.each(function() {
					...
					$.data($el, ns + '_plugin', null); // remove from element's data
				});
			}
			...
			this.each(function() {
				...
				if (r !== null && r !== undefined) return false; // break
			});
		...
		return this.each(function() {get($(this), options)});
	};
